const CONFIG = {
  ...
  SHEETS: {
    ...
    // hogar=1B47 finanzas=1eI4 — confirmado por consola
...
function showProyectoDetail(idx) {
  ...
  var subs=DATA.remo.filter(function(x){return x["Área"]===p["Área"]&&x!==p;}).slice(0,5);
  ...
  html+=field("Fecha Fin Real",p["Fecha Fin Real"]);
  html+='</div>';
  if(p["Notas / Dependencias"]) html+='<div class="det-section"><div class="det-section-title">Notas / Dependencias</div><div class="det-notes">'+p["Notas / Dependencias"]+'</div></div>';
  html+='<div class="det-section"><div class="det-section-title">Etapa actual</div>';
  var etapas=["Descripcion y Prioridad","Diseno","How-To","Mano de Obra","Materiales","Precios","Programacion","Pruebas","Seguimiento","Cierre"];
  ...
  etapas.forEach(function(e,i){
    var active=i===etapa;
    html+='<div style="padding:4px 10px;border-radius:99px;font-size:11px;background:'+(active?colors[i]+"33":"#0f172a")+';color:'+(active?colors[i]:"#64748b")+';border:1px solid '+(active?colors[i]:"#334155")+'">'+( i+1)+'. '+e+'</div>';
  });
  html+='</div></div>';
  if(subs.length>0){
    html+='<div class="det-section"><div class="det-section-title">Otros proyectos en '+p["Área"]+'</div>';
    subs.forEach(function(s){html+='<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #1e293b;cursor:pointer" onclick="closeDetail();setTimeout(function(){showProyectoDetail(DATA.remo.indexOf(s));},50)"><span style="font-size:13px">'+s["Nombre del Proyecto"]+'</span>'+bdg(s["Estado"]||"Pendiente",ec(s["Estado"]))+'</div>';});
    html+='</div>';
  }
  openDetail(html);
}
...
function showTareaDetail(idx) {
  ...
  var subs=DATA.hogar.subtareas.filter(function(s){return s["ID Tarea Padre"]===t["ID_Tarea"]||s["ID Tarea Padre"]===t["ID_Tarea"];});
  var html='<div class="det-header"><div class="det-back" onclick="closeDetail()">← Hogar</div><div class="det-title">'+(t["Descripción"]||"—")+'</div><div class="det-sub">'+(t["Categoría"]||"—")+( t["Subcategoría"]?" · "+t["Subcategoría"]:"")+'</div></div>';
  ...
  html+=field("Fecha Creacion",t["Fecha Creación"]||t["Fecha Creacion"]);
  ...
  if(subs.length>0){
    ...
    subs.forEach(function(s){
      ...
      html+='<div><div style="font-size:13px">'+(s["Descripción Subtarea"]||s["Descripcion Subtarea"]||"—")+'</div>';
      if(s["Responsable"]) html+='<div style="font-size:11px;color:#64748b">'+s["Responsable"]+'</div>';
      html+='</div>'+bdg(s["Estado"]||"Pendiente",ec(s["Estado"]))+'</div>';
    });
    html+='</div>';
  }
  openDetail(html);
...
function showArticuloDetail(idx) {
  ...
  var etapa=+(a["Etapa"]||0);
  html+='<div class="det-section"><div class="det-section-title">Pipeline de compra</div><div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:8px">';
...
function showGastoDetail(idx) {
  ...
  html+=field("Subcategoria",g["Subcategoría"]||g["Subcategoria"]);
  html+=field("Metodo de pago",g["Método de pago"]||g["Metodo de pago"]);
  html+=field("Banco/TC",g["Banco/TC"]);
  html+=field("Establecimiento",g["Establecimiento"]);
  html+=field("MSI",g["MSI"]);
  html+=field("Meses",g["Meses"]);
  html+='</div>';
  if(g["ComprobanteDrive"]||g["Comprobante Drive"]){
    var url=g["ComprobanteDrive"]||g["Comprobante Drive"];
    html+='<div class="det-section"><a href="'+url+'" target="_blank" style="color:#60a5fa;font-size:13px">Ver comprobante →</a></div>';
  }
...
function renderDashboard(){
  ...
  var tVenc=tPend.filter(function(t){if(!t["Fecha Límite"]&&!t["Fecha Limite"])return false;var fl=new Date(t["Fecha Límite"]||t["Fecha Limite"]);fl.setHours(0,0,0,0);return fl<=hoyD;});
  ...
  document.getElementById("dash-metrics").innerHTML=
    ...
    '<div class="metric-card" style="--accent:'+sc+'"><div class="metric-label">Saldo conciliado</div><div class="metric-val">'+fmt(saldoConc)+'</div><div class="metric-sub">'+fmt(totalCuentas)+' en cuentas</div></div>'+
    '<div class="metric-card" style="--accent:#fbbf24"><div class="metric-label">Gastos del mes</div><div class="metric-val">'+fmt(totG)+'</div><div class="metric-sub">'+gMes.length+' movimientos</div></div>'+
  ...
  document.getElementById("dash-estados").innerHTML='<div class="sec">Avance por estado</div>'+(tot===0?ediv("Sin proyectos"):
  ...
  document.getElementById("dash-prioridad").innerHTML='<div class="sec">Prioridad alta</div>'+(aP.length===0?ediv("Sin items de prioridad alta 🎉"):
    aP.slice(0,10).map(function(p,i){
      var nombre=p["Nombre del Proyecto"]||p["Descripción"]||p["Nombre"]||"—";
      var zona=p["Área"]||p["Categoría"]||"Hogar";
      return'<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid #1e293b;cursor:pointer" onclick="'+(p["Área"]?'showPage(\'remodelacion\',null);setTimeout(function(){showProyectoDetail(DATA.remo.indexOf(DATA.remo.filter(function(x){return x[\'Nombre del Proyecto\']==\''+nombre+'\'})[0]));},100)':'showPage(\'hogar\',null);setTimeout(function(){showTareaDetail(DATA.hogar.tareas.indexOf(DATA.hogar.tareas.filter(function(x){return x[\'Descripción\']==\''+nombre+'\'})[0]));},100)')+'"><div style="font-size:13px">'+nombre+'</div>'+bdg(zona,"#f87171")+'</div>';
    }).join(""));
}
...
function filterRemo(){
  ...
  document.getElementById("remo-tbody").innerHTML=f.length===0?erow(8,"Sin resultados"):
    f.map(function(p,i){
      var realIdx=DATA.remo.indexOf(p);
      return'<tr style="cursor:pointer" onclick="showProyectoDetail('+realIdx+')">'+
        '<td style="color:#94a3b8">'+(p["Área"]||"—")+'</td>'+
        '<td style="font-weight:500">'+(p["Nombre del Proyecto"]||"—")+'</td>'+
        ...
        '<td style="color:#64748b">'+(p["Contratista"]||"—")+'</td></tr>';
    }).join("");
}
...
function renderHogar(){
  ...
  var vencidas=pend.filter(function(t){if(!t["Fecha Límite"]&&!t["Fecha Limite"])return false;var fl=new Date(t["Fecha Límite"]||t["Fecha Limite"]);fl.setHours(0,0,0,0);return fl<=hoyD;});
  ...
  var proximas=pend.filter(function(t){if(!t["Fecha Límite"]&&!t["Fecha Limite"])return false;var fl=new Date(t["Fecha Límite"]||t["Fecha Limite"]);fl.setHours(0,0,0,0);var d=Math.round((fl-hoyD)/(1000*60*60*24));return d>=0&&d<=3;});
  ...
  document.getElementById("hogar-tareas-tbody").innerHTML=tareas.length===0?erow(6,"Sin tareas — dile al agente que agregue tareas"):
    tareas.map(function(t,i){
      var nombre=t["Nombre"]||t["Descripción"]||t["Descripcion"]||"—";
      var fechaLimite=t["Fecha Límite"]||t["Fecha Limite"]||"";
      var dias=0;var nivelColor="#64748b";var diasLabel="";
      if(fechaLimite&&t["Estado"]&&t["Estado"].indexOf("Completo")===-1){
        var fl=new Date(fechaLimite);fl.setHours(0,0,0,0);
        ...
        else if(dias>=-3){nivelColor="#fbbf24";diasLabel='<span style="color:#fbbf24;font-size:10px"> 🟡 '+Math.abs(dias)+'d</span>';}
      }
      return'<tr style="cursor:pointer;'+(dias>0?'border-left:2px solid '+nivelColor+';':'')+'" onclick="showTareaDetail('+i+')">'+
        '<td style="color:#94a3b8">'+(t["Categoría"]||t["Categoria"]||"—")+'</td>'+
        '<td style="font-weight:500">'+nombre+diasLabel+'</td>'+
        '<td>'+bdg(t["Prioridad"]||"Media",pc(t["Prioridad"]))+'</td>'+
        '<td>'+bdg(t["Estado"]||"—",ec(t["Estado"]))+'</td>'+
        '<td style="color:#94a3b8">'+(t["Asignado A"]||"—")+'</td>'+
        '<td style="color:'+(dias>0?nivelColor:"#64748b")+';font-weight:'+(dias>0?600:400)+'">'+( fechaLimite||"—")+'</td></tr>';
    }).join("");
  ...
  document.getElementById("hogar-subtareas-list").innerHTML=keys.length===0?ediv("Sin subtareas pendientes 🎉"):
    keys.map(function(k){
      ...
      return'<div style="margin-bottom:10px"><div style="font-size:13px;font-weight:500;color:#f1f5f9;margin-bottom:4px">'+nombre+'</div>'+
        grouped[k].map(function(s){return'<div class="sub-row"><span>'+(s["Descripción Subtarea"]||s["Descripcion Subtarea"]||"—")+'</span>'+bdg(s["Estado"]||"Pendiente",ec(s["Estado"]))+'</div>';}).join("")+'</div>';
    }).join("");
  ...
  var calleHtml=calleItems.length===0?ediv("Sin pendientes que requieran salir 🎉"):
    (function(){var z={};calleItems.forEach(function(t){var zn=t["Zona"]||"Sin zona";if(!z[zn])z[zn]=[];z[zn].push(t);});
      return Object.keys(z).map(function(zn){
        return'<div style="margin-bottom:14px"><div style="font-size:12px;font-weight:600;color:#60a5fa;margin-bottom:6px">📍 '+zn.toUpperCase()+'</div>'+
          z[zn].map(function(t){return'<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #1e293b"><span style="font-size:13px">'+(t["Descripción"]||t["Tipo Tramite"]||"—")+'</span>'+bdg(t["Prioridad"]||"Media",pc(t["Prioridad"]))+'</div>';}).join("")+'</div>';
      }).join("")+'<div style="padding:10px;background:#0f172a;border-radius:8px;border:1px solid #334155;font-size:13px">'+calleItems.length+' pendiente(s) en '+Object.keys(z).length+' zona(s)</div>';
    })();
  ...
  document.getElementById("hogar-mascotas-tbody").innerHTML=DATA.hogar.mascotas.length===0?erow(6,"Sin registros"):
  ...
  document.getElementById("hogar-tramites-tbody").innerHTML=DATA.hogar.tramites.length===0?erow(6,"Sin tramites"):
...
function renderFinanzas(){
  ...
  document.getElementById("fin-metrics").innerHTML=
    '<div class="metric-card" style="--accent:#4ade80"><div class="metric-label">Total en cuentas</div><div class="metric-val">'+fmt(totalCuentas)+'</div><div class="metric-sub">'+cuentas.length+' activa(s)</div></div>'+
  ...
  if(cuentas.length===0){html+=ediv("Sin cuentas — dile al agente: \"agrega cuenta efectivo con saldo $X,XXX\"");}
  else{cuentas.forEach(function(c){html+='<div style="display:flex;justify-content:space-between;margin-bottom:6px"><span style="color:#94a3b8">'+(c["Nombre"]||"Cuenta")+' <span style="font-size:11px;color:#64748b">['+( c["Tipo"]||"")+']</span></span><span style="color:#4ade80;font-weight:600">'+fmt(c["SaldoActual"])+'</span></div>';});}
  ...
  if(tarjetas.length===0){html+='<div style="color:#64748b;font-size:12px;margin-bottom:8px">Sin tarjetas —